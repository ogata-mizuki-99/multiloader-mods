...
public class ShopScreen extends Screen {
    ...
    private void refreshShopData(boolean showLoading) {
        ...
        EconomyService.fetchShopDetails(this.shopId, playerUuid).thenAccept(response -> {
            ...
            try {
                ...
                EconomyCommon.LOGGER.error("Failed to parse shop response: ", e);
                if (showLoading) {
                    Minecraft.getInstance().execute(() -> {
    ...
    private boolean canBuy(ShopItemData item, int quantity) {
        if (isProcessing || item.shopItemId == null || item.buyPrice <= 0)
            return false;
        if (EconomyCommon.getCurrentBalance() < item.buyPrice * (long) quantity)
            return false;
    ...
    private int computeTradeStateSignature() {
        int sig = EconomyCommon.getCurrentBalance();
    ...
    private void handleBuy(ShopItemData item, int quantity) {
        ...
        EconomyPlatform.sendToServer(new ShopBuyRequestPayload(item.shopItemId, quantity));
    }
    ...
    private void handleSell(ShopItemData item, int quantity) {
        ...
        EconomyPlatform.sendToServer(new ShopSellRequestPayload(item.itemId, quantity));
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        String playerMoneyText = EconomyMasterI18n
                .tr("economy.ui.balance", EconomyMasterI18n.formatCurrency(EconomyCommon.getCurrentBalance())).getString();
        ...
        for (int i = startIdx; i < endIdx; i++) {
            ...
            try {
                ...
                if (!stack.isEmpty()) {
                    ...
                    EconomyCommon.LOGGER.warn("Item not found in registry: " + item.itemKey);
                }
            } catch (Exception e) {
                EconomyCommon.LOGGER.error("Failed to load item for icon: " + item.itemKey, e);
            }
