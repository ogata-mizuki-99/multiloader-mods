...
public class LoanScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addRenderableWidget(Button.builder(EconomyMasterI18n.tr("economy.ui.loan.repay_all"), button -> {
            int debt = EconomyCommon.getCurrentDebt();
    ...
    private void handleBorrow() {
        ...
        if (mc.player != null) {
            ...
            updateButtonStates();
            EconomyPlatform.sendToServer(new LoanRequestPayload("BORROW", this.currentAmount));
        }
    }
    ...
    private void handleRepay() {
        ...
        if (mc.player != null) {
            this.isProcessing = true;
            updateButtonStates();
            EconomyPlatform.sendToServer(new LoanRequestPayload("REPAY", this.currentAmount));
        }
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        String balanceText = EconomyMasterI18n
                .tr("economy.ui.loan.balance", EconomyMasterI18n.formatCurrency(EconomyCommon.getCurrentBalance()))
                .getString();
        String debtText = EconomyMasterI18n
                .tr("economy.ui.loan.debt", EconomyMasterI18n.formatCurrency(EconomyCommon.getCurrentDebt())).getString();
        ...
        if (this.currentAmount > 0) {
            ...
            int previewColor = isLimitLoaded
                    && Math.round(this.currentAmount * 1.1) + EconomyCommon.getCurrentDebt() > maxAllowedDebt
                            ? 0xFFFF5555
                            : 0xFFFFBB33;
