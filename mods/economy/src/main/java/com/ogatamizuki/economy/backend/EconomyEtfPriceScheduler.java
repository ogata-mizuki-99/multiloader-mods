...
public final class EconomyEtfPriceScheduler {
    ...
    public static void start(MinecraftServer server) {
        ...
        if (!EconomyFeatures.isEtfUpdatesEnabled()) {
            EconomyMod.LOGGER.info("ETF random walk scheduler skipped (enableEtfUpdates=false)");
            return;
        }
        ...
        task = scheduler.scheduleAtFixedRate(() -> {
            try {
                ...
                EconomyMod.LOGGER.warn("ETF random walk task failed: {}", e.getMessage());
            }
        }, intervalMinutes, intervalMinutes, TimeUnit.MINUTES);
        EconomyMod.LOGGER.info("ETF random walk scheduled every {} minutes", intervalMinutes);
    }
