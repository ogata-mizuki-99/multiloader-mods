...
public class ClientAccess {
    ...
    public static CompletableFuture<String> requestQuery(String queryType, String arg1, int arg2) {
        ...
        Minecraft.getInstance().getConnection().send(new EconomyQueryRequestPayload(queryType, arg1, arg2));
        return future;
    }
    ...
    private static CompletableFuture<String> requestMasterItemsPage(int page, JsonArray accumulated, String sourceHint) {
        return requestQuery("MASTER_ITEMS", "", page).thenCompose(json -> {
            ...
            try {
                ...
                EconomyMod.LOGGER.error("Failed to assemble MASTER_ITEMS pages", e);
                return CompletableFuture.completedFuture("null");
            }
        });
    }
    ...
    public static CompletableFuture<Void> requestDepositBank(int amount) {
        ...
        Minecraft.getInstance().getConnection().send(new BankRequestPayload("DEPOSIT", amount));
        return pendingBankRequest;
    }

    public static CompletableFuture<Void> requestWithdrawBank(int amount) {
        pendingBankRequest = new CompletableFuture<>();
        Minecraft.getInstance().getConnection().send(new BankRequestPayload("WITHDRAW", amount));
        return pendingBankRequest;
    }
    ...
    public static CompletableFuture<String> requestShopDetails(int shopId) {
        ...
        Minecraft.getInstance().getConnection().send(new ShopDetailsRequestPayload(shopId));
        return future;
    }
    ...
    public static void registerRenderers(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(EconomyMod.ECONOMY_NPC.get(), EconomyNpcRenderer::new);
        event.registerEntityRenderer(EconomyMod.LOAN_NPC.get(), LoanNpcRenderer::new);
    }
