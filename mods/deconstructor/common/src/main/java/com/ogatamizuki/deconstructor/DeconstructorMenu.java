...
public class DeconstructorMenu extends AbstractContainerMenu {
    ...
    public DeconstructorMenu(int containerId, Inventory playerInventory, BlockPos pos) {
        this(containerId, playerInventory, playerInventory.player.level().getBlockEntity(pos) instanceof DeconstructorBlockEntity be ? be : null);
    }

    public DeconstructorMenu(int containerId, Inventory playerInventory, DeconstructorBlockEntity blockEntity) {
        super(DeconstructorBlocks.DECONSTRUCTOR_MENU_TYPE.get(), containerId);
        this.blockEntity = blockEntity;
        this.level = playerInventory.player.level();
        this.maxExtractCount = blockEntity != null ? blockEntity.getMaxExtractCount() : 1;
    ...
    public void slotsChanged(net.minecraft.world.Container container) {
        ...
        if (!level.isClientSide() && !isUpdatingRecipe) {
            ...
            ItemStack cached = blockEntity != null ? blockEntity.getInputStack() : ItemStack.EMPTY;

            // 入力スロットが空になった場合（ただし、回収がまだ始まっていない場合のみクリアする）
            if (input.isEmpty() && getCurrentExtractCount() == 0) {
                if (blockEntity != null) blockEntity.setInputStackIfChanged(ItemStack.EMPTY);
            ...
            if (!input.isEmpty() && !isSame) {
                if (blockEntity != null) blockEntity.setInputStackIfChanged(input.copy());
                setCurrentExtractCount(0);
                updateRecipePreview();
                return;
            }
        ...
        try {
            ...
            if (currentCount == 1) {
                ...
                if (!input.isEmpty()) {
                    ...
                    if (blockEntity != null) blockEntity.setInputStackIfChanged(remaining.copy());
                }
            }
    ...
    /**
     * Returns the cached input stack from blockEntity, or from the container slot directly
     * (used when blockEntity is null, e.g. NeoForge client-side MenuType construction).
     */
    private ItemStack getCachedInput() {
        if (blockEntity != null) {
