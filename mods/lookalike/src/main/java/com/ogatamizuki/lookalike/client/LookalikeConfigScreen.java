 ...
 */
public final class LookalikeConfigScreen extends Screen {
    private final ModContainer container;
    private final Screen parent;

    public LookalikeConfigScreen(ModContainer container, Screen parent) {
    ...
    protected void init() {
        ...
        this.addRenderableWidget(Button.builder(
                Component.translatable("lookalike.configuration.open_editor"),
    ...
    private void pushToServer() {
        ...
        if (mc.getConnection() == null) {
            mc.player.sendSystemMessage(
                    Component.translatable("lookalike.configuration.push_need_connection")
                            .withStyle(ChatFormatting.RED));
            return;
        }
        mc.getConnection().send(LookalikeCommonConfigPushPayload.fromLocalConfig());
        mc.player.sendSystemMessage(
                Component.translatable("lookalike.configuration.push_sent").withStyle(ChatFormatting.YELLOW));
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.centeredText(
                this.font,
                Component.translatable("lookalike.configuration.push_hint"),
                this.width / 2,
                this.height / 2 - 40,
                0xFFAAAAAA);
