...
public class DisguiseMirrorItem extends Item {
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity interactionTarget, InteractionHand hand) {
        ...
        if (!player.level().isClientSide()) {
            ...
            LookalikeAttachments.ScanHistory history = LookalikePlatform.getHistory(serverPlayer);
            history.addEntry(
                    serverTarget.getUUID(),
                    DisguiseManager.getStoredGameProfile(serverTarget).name()
            );
            LookalikePlatform.setHistory(serverPlayer, history);
            DisguiseManager.syncScanHistory(serverPlayer);
