...
public class StructureExportScreen extends Screen {
    ...
    private void submit() {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null || mc.getConnection() == null || this.nameBox == null) {
            return;
        }
        ...
        submitted = true;
        mc.getConnection().send(new ExportSubmitPayload(name, category));
        super.onClose();
    }
    ...
    public void onClose() {
        Minecraft mc = Minecraft.getInstance();
        if (!submitted && mc.getConnection() != null) {
            mc.getConnection().send(new ExportCancelPayload(false));
        }
        super.onClose();
    }
