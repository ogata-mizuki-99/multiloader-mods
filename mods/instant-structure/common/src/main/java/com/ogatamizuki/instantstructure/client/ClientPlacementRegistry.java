...
import com.ogatamizuki.instantstructure.InstantStructureCommon;
...
public class ClientPlacementRegistry {
    ...
    /** 設置失敗時: 仮確定を解除し、プレビューを照準追従モードへ戻す。 */
    public static void onBuildRejected() {
        if (!active) {
            return;
        }
        clearTentative();
    ...
    public static boolean isHoldingBuilder(Minecraft mc) {
        ...
        return mc.player.getMainHandItem().is(InstantStructureCommon.INSTANT_BUILDER.get())
                || mc.player.getOffhandItem().is(InstantStructureCommon.INSTANT_BUILDER.get());
    }

    /** インスタントビルダーを手から離したら配置プレビューを終了する。 */
    public static void syncHeldItem(Minecraft mc) {
        if (active && !isHoldingBuilder(mc)) {
            reset();
        }
    }

    public static boolean isPlacementVisible(Minecraft mc) {
        return active && isHoldingBuilder(mc);
    }
